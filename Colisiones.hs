...
checkCollision r1 r2 = all (checkCollisionEje r1 r2) norms        -- Hay colisión si en todas las normales de las aristas se solapan las proyecciones
...
rectTanque Objeto { posicion = p, tamanyo = t, extra = a } = getVertices (trasladoRect (rectangulo t) p) (anguloCannon a)

-- Calcula la hitbox de una bala teniendo en cuenta su posición y dirección a la que se dirige
rectBala :: Bala -> Rect
rectBala Objeto { posicion = p, tamanyo = t, velocidad = v } = getVertices (trasladoRect (rectangulo t) p) (angleToTarget (0, 0) v)
...
-- Actualizar la vida de un tanque cuando es impactado por una bala
actualizarVidaTanque :: Tanque -> Bala -> Tanque
actualizarVidaTanque t b =
    let energiaActual = energia (extra t)
        danyoBala = danyo (extra b)
        nuevaEnergia = max 0 (energiaActual - danyoBala)
        infoActual = extra t
        infoNueva = infoActual { energia = nuevaEnergia }
